 ...
 * to allow tracking if any of them is currently loading or not.
 *
...
public class CachedFieldsListener implements FieldStateListener, EndpointStateListener {
    ...
    public synchronized boolean getCurrentState() {
        ...
        List<CachedField> currentFieldsNoArgs = new ArrayList<CachedField>(fieldsNoArgs);
        List<CachedFieldWithArg> currentFields = new ArrayList<CachedFieldWithArg>(fields);
        List<CachedEndpointWithArg> currentEndpoints = new ArrayList<CachedEndpointWithArg>(endpoints);
